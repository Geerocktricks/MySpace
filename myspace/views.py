...
from .models import Post,NewsLetterRecipients,Category, update_post_id_field_in_category_model,tags
...
def post_list_view(request, category_slug=None):
    ...
    return render(request, 'blog/post/list.html', {'posts': posts,"letterForm":form, 'recent': recent, 'categories': categories, 'post': post, 'twopercategory': twopercategory, 'tagz':tagz})
...
def post_detail_view(request, year, month, day, post, category_slug=None):
    ...
    if request.method == 'POST':
        ...
        form = NewsLetterForm()
    return render(request, 'blog/post/detail.html', {'post': post,"letterForm":form, 'categories': categories, 'recent': recent,})
